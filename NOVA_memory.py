"""Simple persistent memory for NOVA.

Stores messages (user and assistant) in a SQLite database under NOVA_data/
...
DB_FILENAME = 'NOVA_memory.db'
DEFAULT_RETENTION_DAYS: Optional[int] = None  # None == keep forever


def _db_path() -> str:
    d = os.path.join(os.getcwd(), 'NOVA_data')
...
def _data_dir_path() -> str:
    ...
    This is separate from `NOVA_data/` and will contain a human-readable
...
def summarize_recent(n: int = 10) -> str:
    ...
    for m in reversed(msgs):
        sender = 'You' if m['sender'] == 'user' else 'NOVA'
