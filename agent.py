...
from NOVA_prompts import behavior_prompts, Reply_prompts
from NOVA_google_search import google_search, get_current_datetime
from NOVA_get_whether import get_weather
from NOVA_window_CTRL import open, close, folder_file
from NOVA_file_opner import Play_file
...
try:
    from NOVA_memory import save_conversation, recall_last, summarize_recent
except Exception:
...
class Assistant(Agent):
    ...
    async def on_reply(self, user_input, NOVA_reply):
        try:
            await asyncio.to_thread(save_conversation, str(user_input), str(NOVA_reply))
        except Exception:
            pass
...
async def entrypoint(ctx: agents.JobContext):
    ...
    async def safe_generate_reply(sess, instructions, retries: int = 3, base_wait: float = 1.0) -> bool:
        """Try to call sess.generate_reply with retries and log failures to NOVA_data/realtime_errors.log.

        Returns True on success, False if all retries failed.
        """
        logdir = os.path.join(os.getcwd(), 'NOVA_data')
    ...
    if not ok:
        print("Failed to generate initial reply after retries. See NOVA_data/realtime_errors.log for details.")
...
if __name__ == "__main__":
    ...
    def start_gui_if_needed():
        try:
            data_dir = os.path.join(os.getcwd(), 'NOVA_data')
            ...
            gui_script = os.path.join(os.getcwd(), 'NOVA_gui.py')
            env = os.environ.copy()
