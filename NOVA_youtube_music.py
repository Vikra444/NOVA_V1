...
class VoiceYouTubeController:
    ...
    def __init__(self):
        ...
        print("Ready to receive commands. Say 'NOVA' followed by your command.")
    ...
    def play_song(self, song_name: str):
        ...
        if not song_name:
            print("NOVA: Please specify a song to play.")
            return

        print(f"NOVA: Playing '{song_name}' on YouTube.")
        try:
            pywhatkit.playonyt(song_name)
            ...
            if self._find_and_focus_youtube():
                ...
                print("NOVA: Playback started. Ready for next command.")
            else:
                print("NOVA: Could not focus the YouTube window for further control.")
        except Exception as e:
            print(f"NOVA: An error occurred while trying to play the song: {e}")
            try:
                with open(data_path('youtube_error.log'), 'a', encoding='utf-8') as lf:
    ...
    def _execute_action(self, key_or_hotkey, response_text):
        ...
        if self._find_and_focus_youtube():
            ...
            print("NOVA: Action failed. Could not find the YouTube window.")
    ...
    def listen(self):
        ...
        while True:
            try:
                with self.microphone as source:
                    print("\nListening for activation word 'NOVA'...")
                ...
                if "NOVA" in text:
                    command = text.split("NOVA", 1)[1].strip()
                    ...
                    if command.startswith("play "):
                        ...
                        print("NOVA: Deactivating. Goodbye!")
                        sys.exit()
                    else:
                ...
                print("\nNOVA: Shutting down on user request.")
                break
            except Exception as e:
